...
export class Computation extends Node implements NodeContext {
  code: string;
  ...
  in<T>(name: string, defaultValue?: T, options: PortOptions = {}): InputPort<T> {
    ...
    // Create new port with index-based ID
  ...
  toJSON() {
    ...
    // Inputs and outputs are not serialized - they are defined in node code
    // and will be recreated when the node code executes
    
    // Only include props if not empty
    const props = Object.entries(this.props).reduce((acc, [key, prop]) => {
      let value = prop.value;
      // Convert color values to array format [r, g, b, a]
      if (prop.type === 'color' && isColorValue(value)) {
        const normalized = normalizeColor(value as any);
        // Always use array format [r, g, b, a] with alpha defaulting to 1.0
