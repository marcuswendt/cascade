...
export interface Attribute {
  ...
  /** Components per element. `P` is 3, `Cd` is 4, `width` is 1. */
...
Wiring one turned out to be the part that did not work. The `input_0`, `input_1`, ... spelling that Studio and saved graphs use for a variadic port was translated by `runtime.ts` only when the target's module id was `cascade.core.Switch` or `cascade.core.Merge`, so `cascade.geo.Merge` could not be connected at all. Ruled a bug by Marcus and fixed generally; see the three rulings below.

`Transform` (Transform SOP). Applies a `mat3` to `P`. Trivial. Uses the existing `mat3` type, which is already documented as column-major. As built it takes Houdini's four parameters — translate, rotate, scale, pivot — and builds the matrix itself, because a matrix-typed input is the primitive the operation wants and the wrong thing to put in front of an artist. Its port is `geometry` in and `geometry` out, which needed the namespace ruling below.
...
### Three rulings from step 3, all of them the runtime refusing something the design needs

Step 3 landed the six nodes and produced three complaints, ruled on by Marcus 2026-09-04. They are one shape: a place where the runtime said no to something the geometry model requires, and in two of the three the node set had already worked around it in a way that was visible in the code.

**A definition may use one name for both an input and an output.** *"Sounds weird, we need to allow this case."* `validate.ts` shared a single name Map across `inputs`, `outputs` and `props`, so `geometry` in and `geometry` out raised `name-collision` and every operator in the set had to call its input `input` — the workaround is in the comment `Transform` used to carry. Outputs now have their own namespace. Inputs and props keep sharing one, and that asymmetry is the substance of the ruling rather than caution: an output is addressed from the other direction, so the format names the two endpoints of a connection separately, the runtime keys them as `source.outputName` and `target.inputName`, and `NodeExecutionContext` hands a node `context.inputs` and `context.outputs` as separate records. An input and a prop have no direction to tell them apart by, they are the same control to whoever is looking at the node, and the legacy engine already *uses* an input sharing a prop's name to mean "this prop is promoted to a pin" (`port.fromParameter`). The expression language resolves a bare `node/name` path against props only, so allowing that pair would make a name-addressed public API ambiguous with no syntax to disambiguate it. That one stays a collision until there is a reason to design a qualified path.

**Variadic wiring is a property of a definition, not of a module id.** *"That is a bug, fix this properly as a general solution."* The `input_N` translation was gated on the target being `cascade.core.Switch` or `cascade.core.Merge`, which made `cascade.geo.Merge` unwireable and would have made every future variadic node unwireable too. It now reads `variadic: true` off the definition, so any node accepts `input_N` whoever wrote it. A definition with two variadic inputs makes `input_N` genuinely ambiguous and the connection is refused rather than guessed at; none declares two today.

**Colour is a four-component vector, always.** *"No, colour should only be a 4 component vector; when a hex/string value is needed we should use conversions with a .toHex and .toString methods."* Step 3 had shipped two exceptions for the convenience of the migration: `Cd` was allowed to be a string attribute so `cloud-plots`' `"#3a7f5c"` could be stored verbatim, and `SvgExport`'s stroke and fill were SVG paint strings so `fill="none"` was expressible. Both are gone. A `Color` namespace in `packages/contracts` owns `fromHex`, `toHex`, `toString` and the reader for an attribute's flat array, and the SVG writer is the only place a colour becomes text. It is a namespace object rather than methods because a colour is a `Vec4` — a plain array, which is what lets it be a graph value, a `vec4` port and a slice of a `Float64Array` at once — and giving it methods would mean a colour that does not survive JSON.

The one design question inside that ruling was what "no fill" is. **A `fillMode` prop of `none` or `solid` beside a `vec4` `fill`**, because a zero-alpha fill and SVG's `fill="none"` are not the same instruction: `none` says the region is not filled, a transparent fill says it is filled with nothing, and a plotter driver reading the second still generates a fill toolpath. Inferring the mode from alpha also throws the colour away, so a fill cannot be switched back on without picking it again. This is also what the style vocabulary below had already independently arrived at — `fillMode` string and `fill` vec4 sit next to each other in that list.
...
One correction to that, from the colour ruling below: the props are `stroke` (a `vec4`), `strokeWidth`, `opacity`, `fill` (a `vec4`) and `fillMode`. `Cd` is the attribute and `stroke` is its fallback, which is the same rule with the SVG attribute's name on the prop; and a `Cd` that is a string, or numeric with any size but four, is refused rather than ignored, because silently dropping a colour attribute is the bug this project already carries twice.
